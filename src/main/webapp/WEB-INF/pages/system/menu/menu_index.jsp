...
<head>
	...
	<script type="text/javascript">
		...
		function searchMenu(){
			...
			$("#edit-menu").linkbutton("disable");
			$("#get-menu").linkbutton("disable");
			var menuStatus = $("#menuStatus").combobox("getValue");//菜单状态
			var menuLevel = $("#menuLevel").combobox("getValue");//菜单状态
			var menuName = $("#menuName").val();//姓名
			...
			$("#tableData").datagrid({   
			    ...
			    columns:[[   
			        ...
			        {field:"menuId",title:"菜单编号",align:"center",width:100},
					{field:"menuName",title:"菜单名称",align:"center",width:100},
					{field:"menuCode",title:"菜单代码",align:"center",width:100},
					{field:"menuFatherId",title:"父菜单节点",align:"center",width:100},
					{field:"menuStatus",title:"菜单状态",align:"center",width:100,formatter: function(value,row,index){
						if(util_status != null){
					...
					{field:"menuUrl",title:"菜单URL",align:"center",width:100},
					{field:"menuIco",title:"菜单图标",align:"center",width:100},
					{field:"menuLevel",title:"菜单等级",align:"center",width:100},
					{field:"menuOrder",title:"菜单排序",align:"center",width:100},
					{field:"menuCreateUserName",title:"创建人",align:"center",width:100},
					{field:"menuCreateTime",title:"创建时间",align:"center",width:100}
			    ]],
			    //查询条件
			    queryParams: {
		...
		/*删除菜单信息*/
		function deleteMenu(){
			$.messager.confirm("菜单模块","您是否确认删除此条数据吗？", function(r){
				if(r){
					var param = getSystemParam();
					...
					$.post(url, { "menuId": menuObject.menuId },
							   function(data){
						   		if(data.result >= 0){
						   			info("菜单模块","删除菜单【"+menuObject.menuName+"】成功！");
						   			self.searchMenu();
						   			//禁用按钮
									$("#delete-menu").linkbutton("disable");
									$("#edit-menu").linkbutton("disable");
							   	}else{
							   		info("菜单模块","删除菜单【"+menuObject.menuName+"】失败！");
								}
					   }, "json");
				}
			});
...
<div data-options="region:'north',title:'系统菜单信息管理',iconCls:'icon-user-list'" border="false"  noheader="true" style="height:auto;padding:0px;">
	<!-- 查询条件 -->
	<div class="form_div" id="form_div" style="width:100%;height:auto;padding:0px; ">
       <form id="paramForm" method="post" name="paramForm" action="">  
			<div style="margin-top:10px;">
				...
				<input class="easyui-textbox trimBlank"  type="text" style="width:70%;height:32px;padding:10px" name="menuName" id="menuName" value=""  data-options="validType:'CN_EN_NO[]',prompt:'请输入菜单名称'">
			</div>
   			<div style="margin-top:10px">
	...
	<div class="form_div"  style="width:100%;height:auto;padding:5px 0px 3px 0px;;text-align:right;">
   		<a  href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-user-search'" id="search-menu" onclick="searchMenu()">查询菜单</a>
   		<a  href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-user-info'" style="display:none;" id="get-menu" onclick="getMenu()">查看详情</a>
    	<a  href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-user-add'" style="display:none;" id="add-menu" onclick="addMenu()">添加菜单</a>  
    	<a  href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-user-edit'" style="display:none;" id="edit-menu" onclick="editMenu()">修改菜单</a>
    	<a  href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-user-remove'" style="display:none;" id="delete-menu" onclick="deleteMenu()">删除菜单</a>
    	<a  href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-factory'" onclick="parent.exit();">退&nbsp;&nbsp;出</a>&nbsp;&nbsp;&nbsp;  
   	</div>
</div>  
<!-- 数据列表 展示 -->
<div data-options="region:'center',iconCls:'icon-user-list',title:'系统菜单信息展示列表'"  style="width:100%;height:100%;padding:0px;margin:0px; ">
