...
<head>
	...
	<script type="text/javascript" src="./commons/js/message.js"></script>
  	<script type="text/javascript">
  	    ...
  	    var buttonId = "${systemButtonDTO.buttonId }";
			...
			//绑定事件
		 	$("#buttonId").textbox("textbox").bind("blur", function(){
				...
				$.post(url,{buttonId:$("#buttonId").val()},function(data){
				    if(data.result==1){
				    	info("按钮管理","按钮已存在！");
				    	$("#buttonId").textbox("setValue",buttonName);
				    	isFind = true;
					}else{
					    isFind = false;
					}
	...
	/*初始化菜单选择框*/
		...
		function update(){
			...
			var param = getSystemParam();
			var url = "button/updateButton.json?param="+param;
...
<body class="easyui-layout" style="padding:0px;margin:0px;" >  
    <div data-options="region:'center',title:'修改按钮',iconCls:'icon-user-add'" >
			 ...
			 <div  class="easyui-panel" noborder="true" border="false" style="width:100%;height:100%;padding:0px;margin:0px;">
		   		 <form id="paramForm" method="post" name="paramForm" action="">  
		   			<div style="margin-top:15px;">
						...
						<input class="easyui-textbox" id="buttonId" type="text" style="width:70%;height:32px;padding:10px" name="buttonId" value="${systemButtonDTO.buttonId }" data-options="readonly:true,prompt:'请输入按钮编号'">
					</div>
		   			<div style="margin-top:15px;">
		   				 <label for="buttonName">按钮名称:</label>
						<input class="easyui-textbox" id="buttonName" type="text" style="width:70%;height:32px;padding:10px" name="buttonName" value="${systemButtonDTO.buttonName }" data-options="prompt:'请输入按钮名称',required:true,validType: ['CN_EN_NO[]','maxLenght[50]']">
					</div>
		   			<div style="margin-top:15px;">
		   				 <label for="buttonCode">按钮编码:</label>
						<input class="easyui-textbox" id="buttonCode" type="text" style="width:70%;height:32px;padding:10px" name="buttonCode" value="${systemButtonDTO.buttonCode }" data-options="prompt:'请输入按钮编码',required:true,validType: ['maxLenght[500]']">
					</div>
		   			<div style="margin-top:15px;">
		   				 <label for="buttonIco">按钮图标:</label>
						<input class="easyui-textbox" id="buttonIco" type="text" style="width:70%;height:32px;padding:10px" name="buttonIco" value="${systemButtonDTO.buttonIco }" data-options="prompt:'请输入按钮图标',validType: ['maxLenght[100]']">
					</div>
		   			<div style="margin-top:15px;">
		   				 <label for="buttonUrl">按钮URL:</label>
						<input class="easyui-textbox" id="buttonUrl" type="text" style="width:70%;height:32px;padding:10px" name="buttonUrl" value="${systemButtonDTO.buttonUrl }" data-options="prompt:'请输入按钮url',validType: ['maxLenght[100]']">
					</div>
		   			<div style="margin-top:15px">
